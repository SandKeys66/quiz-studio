...
<body>
  ...
  <script>
    ...
    function setupDrawingCanvas(disabled){
      ...
      const playerPane = canvas.closest(".player-pane");
      const stopBrowserGesture = event => event.preventDefault();
      const nonPassive = { passive: false };

      playerPane?.addEventListener("selectstart", stopBrowserGesture);
      playerPane?.addEventListener("dragstart", stopBrowserGesture);
      canvas.addEventListener("contextmenu", stopBrowserGesture);

      // iOS Safari が2回目の接触を文字選択やダブルタップ操作として
      // 予約する前に、キャンバス上の標準タッチ動作をキャンセルする。
      canvas.addEventListener("touchstart", stopBrowserGesture, nonPassive);
      canvas.addEventListener("touchmove", stopBrowserGesture, nonPassive);
      canvas.addEventListener("touchend", stopBrowserGesture, nonPassive);
      canvas.addEventListener("touchcancel", stopBrowserGesture, nonPassive);

      // Safari固有のジェスチャーイベントもキャンバス内だけで抑止する。
      canvas.addEventListener("gesturestart", stopBrowserGesture, nonPassive);
      canvas.addEventListener("gesturechange", stopBrowserGesture, nonPassive);
      canvas.addEventListener("gestureend", stopBrowserGesture, nonPassive);
